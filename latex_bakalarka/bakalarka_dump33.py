...
USAGE:
1) Set ROOT_DIR below (absolute path to your cloned repo root) OR leave None to auto-detect git root.
2) Optionally set OUTPUT_DIR to force where the dump is saved.
3) Run: python folder_dump.py
"""
...
# If not None, the dump is ALWAYS saved here (overrides EXPORT_TO_ROOT_DIR).
# Example: r"C:\Users\volny\Desktop"
OUTPUT_DIR: str | None = r"C:\Users\volny\Desktop\skola"
...
# Ignored when OUTPUT_DIR is not None.
...
EXCLUDED_DIR_NAMES = {
    ...
    "phd_thesis",
    "master_thesis",
    # "bachelor_thesis",  # keep ENABLED for your bachelor thesis repo (so DON'T exclude it)
}
...
def build_tree_lines(root: Path) -> list[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for d in list(dirnames):
            if should_exclude_dir(dir_path / d):
                dirnames.remove(d)
...
def resolve_output_dir(root: Path, start: Path) -> Path:
    """
    Priority:
    1) OUTPUT_DIR (explicit)
    2) ROOT_DIR (if EXPORT_TO_ROOT_DIR=True)
    ...
    if OUTPUT_DIR:
        ...
        if not out.is_dir():
            raise SystemExit(f"OUTPUT_DIR is not a directory: {out}")
        return out

    return root if EXPORT_TO_ROOT_DIR else start.resolve()
...
def main() -> None:
    ...
    root = Path(ROOT_DIR).expanduser().resolve() if ROOT_DIR else find_repo_root(start)

    if not root.exists() or not root.is_dir():
        raise SystemExit(f"ROOT_DIR does not exist or is not a directory: {root}")

    out_dir = resolve_output_dir(root, start)
    ...
    with out_path.open("w", encoding="utf-8", newline="\n") as f:
        ...
        f.write(f"RESOLVED_ROOT: {root}\n")
        f.write(f"RESOLVED_OUTPUT: {out_path}\n")
        f.write(f"{'=' * 80}\n")
